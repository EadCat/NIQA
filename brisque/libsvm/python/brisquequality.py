...
def test_measure_BRISQUE(imgroot):
    imglist = []
    imglist += sorted(glob.glob(os.path.join(imgroot, '*.png')))
    imglist += sorted(glob.glob(os.path.join(imgroot, '*.jpg')))
    total_score = 0
